...
def transform_rental_rate(film_df):
	...
	rental_rate_str = film_df.rental_rate.astype("str")
	rental_rate_expanded = rental_rate_str.str.split(".", expand=True)
	return film_df.assign(rental_rate_dollar=rental_rate_expanded[0], rental_rate_cents=rental_rate_expanded[1])
...
# repl:~/airflow/dags$ cat dag_recommendations.py
from airflow.models import DAG
...
def etl(db_engines):
	...
	courses_to_recommend = transform_courses_to_recommend(rating, courses).astype({"course_id": "uint8", "user_id": "uint16"})
	del courses, rating
	recommendations = transform_recommendations(avg_course_rating, courses_to_recommend)
	del avg_course_rating, courses_to_recommend
